...
function getStatus() {
    ...
    ct.getLoanData().then(function(data) {
        ...
        }else if (data[7].c[0] == 3){
            $('#sectionAStatus').html('Settled');
            $('#sectionBStatus').html('Settled');
            $('#sectionCStatus').html('Settled');
            $('#sectionDStatus').html('Settled');
        }
...
function submitLoan() {
    ...
    ct.submitLoan.sendTransaction(_addressOfProperty,
        ...
        insurerAccount, 
        irsAccount,
        ownerAccount,{ from: ownerAccount, gas: defaultGas }
...
function approveLoan() {
    var ct = Mortgage.at(loanContractAddress);
    ct.approveRejectLoan.sendTransaction(2, ownerAccount, { from: bankAccount, gas: defaultGas }).then(function(txHash) {
        getStatus();
        var ct = Mortgage.at(loanContractAddress);
        ct.getBalance(ownerAccount).then(function(data) {
            $('#ownerBalance').html(data.c[0]);
...
function settleLoan(){
    var ct= Mortgage.at(loanContractAddress);
    ct.settleLoan.sendTransaction(3, ownerAccount, { from: bankAccount, gas: defaultGas }).then(function(txHash) {
        getStatus();
        var ct = Mortgage.at(loanContractAddress);
        ct.getLoanData().then(function(data) {
            $('#propOwner').html(data[11]);
...
window.onload = async function() {
    ...
    $("#settleLoan").click(function() {
        settleLoan()
    });
    
    $("valueIRS").on("change",function() {
        truncateToDecimals($("valueIRS"));
    });

    $("valueIssurer").on("change",function() {
        truncateToDecimals($("valueIssurer"));
    });
    
    $("valueMH").on("change",function() {
        truncateToDecimals($("valueMH"));
    });

    $("AT").on("change",function() {
        truncateToDecimals($("AT"));
    });

    $("LA").on("change",function() {
        truncateToDecimals($("LA"));
    });

    $("IR").on("change",function() {
        truncateToDecimals($("IR"));
    });

    $("YR").on("change",function() {
        truncateToDecimals($("YR"));
    });

    $("purchasePrice").on("change",function() {
        truncateToDecimals($("purchasePrice"));
    });

    $("AI").on("change",function() {
        truncateToDecimals($("AI"));
    });

    $("#sectionAAddress").click(function() {
        getLoanData();
    });

    $("#sectionBAddress").click(function() {
        getLoanData();
    });

    $("#sectionCAddress").click(function() {
        getLoanData();
    });

    $("#sectionDAddress").click(function() {
        getLoanData();
    });
